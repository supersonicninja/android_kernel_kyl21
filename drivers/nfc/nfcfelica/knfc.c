...
#if 0
#define RFS_POLL_GPIO_NO			PM8058_GPIO_PM_TO_SYS(MFD_RFS_PM_GPIO_NUM-1)
#else
...
/* static unsigned int intu_poll_poll(struct file *file, poll_table *wait);*/
...
static int felicadev_fsync(struct file *file, int datasync);

static int felicadev_init(void);
static void felicadev_exit(void);
/* static void felicadev_poll_work_func(struct work_struct *work);*/
static void felicadev_poll_kick( void );
...
static unsigned int intu_poll_poll(struct file *file, poll_table *wait)
{
	...
	poll_wait(file, &intu_d->read_wait, wait);
...
static long felicadev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	g_functype = -1;
	up( &chardev_sem );
/*	KNFC_LOG_D("felicadev ioctl END ret = %d cmd = %x",nRet, cmd);*/
	return nRet;
}
...
/* static void felicadev_poll_work_func(struct work_struct *work)
{
	struct icc_poll_data *dummy_d = &g_felicadev_poll_data;

	KNFC_LOG_D("felicadev_poll_work_func START");

	dummy_d->handler_done = 1;
	wake_up_interruptible(&dummy_d->read_wait);

	KNFC_LOG_D("felicadev_poll_work_func END" );
}*/
static void felicadev_poll_kick( void )
{
	struct icc_poll_data *felicadev_poll_d = &g_felicadev_poll_data;

/*	KNFC_LOG_D("feliadev_poll_kick START");*/

	felicadev_poll_d->handler_done = 1;
	wake_up_interruptible(&felicadev_poll_d->read_wait);

/*	KNFC_LOG_D("felicadev_poll_kick END");*/
}
static ssize_t felicadev_poll_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	struct icc_poll_data *felicadev_poll_d = &g_felicadev_poll_data;
...
/*	KNFC_LOG_D("felicadev_poll read START");*/
...
/*	KNFC_LOG_D("felicadev_poll_read END read size = %d ", len);*/
...
/*	KNFC_LOG_D("felicadev_poll_write START len = %d",len);*/
...
/*	KNFC_LOG_D("felicadev_poll_write END");*/
	return 0;
}
static unsigned int felicadev_poll_poll(struct file *file, poll_table *wait)
{
...
/*	KNFC_LOG_D("felicadev_poll_poll START");*/
	
	poll_wait(file, &felicadev_poll_d->read_wait, wait);
	if (0 != felicadev_poll_d->handler_done){
		mask = POLLIN | POLLRDNORM;
	}
/*	KNFC_LOG_D("felicadev_poll_poll END mask = %d", mask);*/
...
/*	INIT_DELAYED_WORK(&g_felicadev_poll_data.work, felicadev_poll_work_func);*/
...
/*	KNFC_LOG_D("cxd2235_poll START");*/
	...
	/*	KNFC_LOG_D("cxd2235_poll() g_cxd_buf[0] = 0x%x ",g_cxd_buf[0]);*/
	/*	KNFC_LOG_D("cxd2235_poll() g_cxd_buf[1] = 0x%x ",g_cxd_buf[1]);*/
	}
	up( &chardev_sem );
/*	KNFC_LOG_D("cxd2235_poll END mask = %d",mask);*/
	return mask;
}
...
/*	KNFC_LOG_D("cxd2235_poll_read START");*/
...
/*	KNFC_LOG_D("cxd2235_poll_read END");*/
	return len;
}
static ssize_t cxd2235_poll_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
...
/*	KNFC_LOG_D("cxd2235_poll_write START");*/
...
/*	KNFC_LOG_D("cxd2235_poll_write END");*/
	return 0;
}
static unsigned int cxd2235_poll_poll(struct file *file, poll_table *wait)
{
...
/*	KNFC_LOG_D("cxd2235_poll_poll START");*/

	poll_wait(file, &cxd_poll_d->read_wait, wait);
	if (0 != cxd_poll_d->handler_done){
		mask = POLLIN | POLLRDNORM;
	}
/*	KNFC_LOG_D("cxd2235_poll_poll END mask = %d", mask);*/
