...
static void msm_spi_process_transfer(struct msm_spi *dd)
{
	...
	if (dd->cur_msg->spi->mode & SPI_NO_CS) {
		...
		gpio_func = (u32)msm_gpio_read_config_func(cs_gpio_num);
		gpio_dir  = (u32)msm_gpio_read_config_dir(cs_gpio_num);
		gpio_pull = (u32)msm_gpio_read_config_pull(cs_gpio_num);
		gpio_drv  = (u32)msm_gpio_read_config_drvstr(cs_gpio_num);
		cs_gpio_enable = 1;
	...
	*/
	...
	if (cs_gpio_enable) {
		...
		gpio_tlmm_config(GPIO_CFG(cs_gpio_num, GPIOMUX_FUNC_GPIO, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, gpio_drv), GPIO_CFG_ENABLE);
	}
...
transfer_end:
	...
	if (cs_gpio_enable) {
		...
		gpio_tlmm_config(GPIO_CFG(cs_gpio_num, gpio_func, gpio_dir, gpio_pull, gpio_drv), GPIO_CFG_ENABLE);
