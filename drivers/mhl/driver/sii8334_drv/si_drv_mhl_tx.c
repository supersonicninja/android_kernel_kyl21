...
#define TX_HW_RESET_PERIOD		1	// 1 ms.
#define TX_HW_RESET_DELAY			1

#define MHL_REGULATOR_MODE_IDLE		0
#define MHL_REGULATOR_MODE_NORMAL	1
#define MHL_REGULATOR_MODE_INIT		0xFF
...
static	bool_t		mhl_queue_flg = false;
static	bool_t		cbuserr_flg = false;
static	uint8_t		mhl_regulator_mode = MHL_REGULATOR_MODE_INIT;
...
#define MHL_REGULATOR_VREG_NAME         "8921_l18"
#define MHL_REGULATOR_CURR_IDLE         (9000)
#define MHL_REGULATOR_CURR_NORMAL       (120000)
...
static struct regulator *mhl_regulator;

struct usb_mhl_event_callback mhlcb;

struct mhl_event_callback *mhl_cb_info=NULL;
int32_t mhl_reg_cbfunc(struct mhl_event_callback* cb)
{
	if (mhl_cb_info == NULL) {
		mhl_cb_info = kzalloc(sizeof(struct mhl_event_callback), GFP_KERNEL);
		if (mhl_cb_info == NULL)
			return -1;
	}
	mhl_cb_info = cb;
	CBUS_DEBUG_PRINT(("MHL:callback func registed.\n"));
	return 0;
}

int32_t mhl_unreg_cbfunc(struct mhl_event_callback* cb)
{
	if (mhl_cb_info != NULL) {
		if (mhl_cb_info == cb) {
			mhl_cb_info = NULL;
			CBUS_DEBUG_PRINT(("MHL:callback func unregisted.\n"));
			return 0;
		}
	}
	return -1;
...
void mhl_device_close(void)
{
	CBUS_DEBUG_PRINT(("MHL:mhl_close_device\n"));
	kfree(mhl_cb_info);
	mhl_cb_info = NULL;
}
...
{
	if(mhl_queue_flg == false) {
		mhl_queue_flg = true;
		queue_work(initializeWorkQueue, &workItem);
		CBUS_DEBUG_PRINT(("MHL:mhl callback event = %d\n",event));
	}
}
...
static void mhl_device_monitoring(struct work_struct *work)
{
	...
	uint8_t intCtrl;
	CBUS_DEBUG_PRINT(("MHL:mhl reset fwPowerState:%02X\n",(int) fwPowerState));
	intCtrl = SiiRegRead(REG_INT_CTRL);
	CBUS_DEBUG_PRINT(("MHL:mhl reset REG_INT_CTRL[0x%08x]\n",intCtrl));
	if(SII_SUCCESS != SiiCraGetLastResult()) {
		CBUS_DEBUG_PRINT(("MHL:mhl reset SII_SUCCESS != SiiCraGetLastResult()\n"));
	} else if((BIT6 & intCtrl) || (true == cbuserr_flg)) {
		/*reset*/
		HalAcquireIsrLock();
		SiiMhlTxInitialize(30);
		HalReleaseIsrLock();
		TX_DEBUG_PRINT(("MHL:mhl reset mhl_device_reset cbuserr_flg = %d \n",  cbuserr_flg));
		cbuserr_flg = false;
	} else {
		CBUS_DEBUG_PRINT(("MHL:mhl reset vbustrigger not\n"));
	}
#endif
	mhl_queue_flg = false;
}
...
void	mhl_device_monitoring_init(void)
{
	...
	initializeWorkQueue = create_workqueue("Sii_initialize_work");
	CBUS_DEBUG_PRINT(("MHL:mhl reset Initialization of workqueue\n"));
	if(initializeWorkQueue == NULL)
		CBUS_DEBUG_PRINT(("MHL:mhl reset Initialization of workqueue failed,\n"));
	INIT_WORK(&workItem, mhl_device_monitoring);

	mhl_regulator = NULL;
	mhl_regulator = regulator_get(NULL, MHL_REGULATOR_VREG_NAME);
}
...
void	mhl_device_monitoring_exit(void)
{
	...
	if (mhl_regulator != NULL)
		regulator_put(mhl_regulator);
}
...
void SiiMhlTxDeviceIsr (void)
{
	...
	do
	{
	if(MHL_REGULATOR_MODE_NORMAL != mhl_regulator_mode) {
		if (mhl_regulator != NULL) {
			ret = regulator_set_optimum_mode(mhl_regulator,
							MHL_REGULATOR_CURR_NORMAL);
			TX_DEBUG_PRINT(("MHL: regulator_set_optimum_mode() called normal. ret=%d\n", ret));
			if (ret < 0)
				TX_DEBUG_PRINT(("MHL: regulator_set_optimum_mode() error. ret=%d\n", ret));
			else
				mhl_regulator_mode = MHL_REGULATOR_MODE_NORMAL;
		}
	}
...
//    			return; // don't do any more I2C traffic until the next interrupt.
	...
	if(POWER_STATE_D3 == fwPowerState) {
		if(MHL_REGULATOR_MODE_IDLE != mhl_regulator_mode) {
			if (mhl_regulator != NULL) {
				ret = regulator_set_optimum_mode(mhl_regulator,
								MHL_REGULATOR_CURR_IDLE);
				CBUS_DEBUG_PRINT(("MHL: regulator_set_optimum_mode() called idle. goto ret=%d\n", ret));
				if (ret < 0)
					TX_DEBUG_PRINT(("MHL: regulator_set_optimum_mode() error. goto ret=%d\n", ret));
				else
					mhl_regulator_mode = MHL_REGULATOR_MODE_IDLE;
			}
		}
	}
...
static void WriteInitialRegisterValues (void)
{
	...
	if (fwPowerState != TX_POWER_STATE_D3)
	{
		...
		CBUS_DEBUG_PRINT(("REG_INT_CTRL [0x%08x]\n",SiiRegRead(REG_INT_CTRL)));
#endif
	...
	if(POWER_STATE_D3 != fwPowerState)
	{
		...
		if(MHL_REGULATOR_MODE_IDLE != mhl_regulator_mode) {
			if (mhl_regulator != NULL) {
				ret = regulator_set_optimum_mode(mhl_regulator,
								MHL_REGULATOR_CURR_IDLE);
				CBUS_DEBUG_PRINT(("MHL: regulator_set_optimum_mode() called idle. ret=%d\n", ret));
				if (ret < 0)
					TX_DEBUG_PRINT(("MHL: regulator_set_optimum_mode() error. ret=%d\n", ret));
				else
					mhl_regulator_mode = MHL_REGULATOR_MODE_IDLE;
...
static int Int4Isr (void)
{
	...
	if(SII_SUCCESS != SiiCraGetLastResult()) {
		...
		{
			if (mhl_cb_info != NULL) {
				CBUS_DEBUG_PRINT(("MHL:callback to otg.\n"));
				mhl_cb_info->fn(1);
			}
		...
		else if(int4Status & BIT3)
		{
			...
			int4Status = SiiRegRead(REG_INTR4);	// read status
			if(SII_SUCCESS == SiiCraGetLastResult()) {
				TX_DEBUG_PRINT(("MHL:mhl reset not D3 state\n"));
				cbuserr_flg = true;
				if(mhl_queue_flg == false) {
					mhl_queue_flg = true;
					queue_work(initializeWorkQueue, &workItem);
					CBUS_DEBUG_PRINT(("MHL:mhl reset cbus\n"));
				}
			}
...
//	    TX_DEBUG_PRINT(("Drv: Clear CBUS INTR_1: %02X\n", (int) cbusInt));
	...
	{
    	uint8_t mscMsg[2];
	    CBUS_DEBUG_PRINT(("Drv: MSC_MSG Received\n"));
		//
	    ...
	    CBUS_DEBUG_PRINT(("Drv: MSC MSG: %02X %02X\n", (int)mscMsg[0], (int)mscMsg[1] ));
    ...
    uint8_t intr[4];
	    ...
	    CBUS_DEBUG_PRINT(("Drv: MHL INTR Received\n"));
...
static void ProcessScdtStatusChange(void)
{
	...
	CBUS_DEBUG_PRINT(("Drv: ProcessScdtStatusChange scdtStatus: 0x%02x\n", scdtStatus));
