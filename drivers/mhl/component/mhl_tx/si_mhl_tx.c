...
static	void		SiiMhlTxMhlCharge(bool_t chargemode);
	...
	else if( mhlTxConfig.mscMsgArrived )
	{
		...
		switch( mhlTxConfig.mscMsgSubCommand )
		{
			...
			case	MHL_MSC_MSG_RCP:
				...
				{
					CBUS_DEBUG_PRINT(("MHL_MSC_MSG_RCP support mscMsgData=%02X \n", (int) ( mhlTxConfig.mscMsgData )));
					AppNotifyMhlEvent(MHL_TX_EVENT_RCP_RECEIVED, mhlTxConfig.mscMsgData);
				}
				else
				{
					TX_DEBUG_PRINT(("MHL_MSC_MSG_RCP unsupport mscMsgData=%02X \n", (int) ( mhlTxConfig.mscMsgData )));
...
void	SiiMhlTxMscCommandDone( uint8_t data1 )
{
    ...
    if ( MHL_READ_DEVCAP == mhlTxConfig.mscLastCommand )
    {
		if (mhlTxConfig.mscLastOffset < sizeof(mhlTxConfig.aucDevCapCache) )
		{
			...
			if((mhlTxConfig.aucDevCapCache[mhlTxConfig.mscLastOffset] == MHL_DEV_CAT_DONGLE_SINK) &&
						(mhlTxConfig.mhlConnected >= MHL_TX_EVENT_CONNECTION)){
				CBUS_DEBUG_PRINT(("MhlTx:SiiMhlTxMscCommandDone MHL_DEV_CAT_SINK\n"));
				SiiMhlTxMhlCharge(true);
			} else if((mhlTxConfig.aucDevCapCache[mhlTxConfig.mscLastOffset] == MHL_DEV_CAT_SELF_POWERED_DONGLE) &&
				(mhlTxConfig.mhlConnected >= MHL_TX_EVENT_CONNECTION)){
				CBUS_DEBUG_PRINT(("MhlTx:SiiMhlTxMscCommandDone MHL_DEV_CAT_SELF_POWERED_DONGLE\n"));
				SiiMhlTxMhlCharge(true);
			} else{
				CBUS_DEBUG_PRINT(("MhlTx:SiiMhlTxMscCommandDone else\n"));
				if(mhlTxConfig.mhlConnected >= MHL_TX_EVENT_CONNECTION){
					SiiMhlTxMhlCharge(false);
				}
			}
...
{
	if(chargemode) {
		TX_DEBUG_PRINT(("MhlTx:pm8921_charger_vbus_draw 1500mA\n"));
		pm8921_set_usb_power_supply_type(POWER_SUPPLY_TYPE_MHL);
		pm8921_charger_vbus_draw(1500);
	} else {
		TX_DEBUG_PRINT(("MhlTx:pm8921_charger_vbus_draw 0mA\n"));
		pm8921_set_usb_power_supply_type(POWER_SUPPLY_TYPE_MHL);
