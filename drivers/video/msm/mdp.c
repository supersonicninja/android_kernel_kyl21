...
static int mdp_probe(struct platform_device *pdev)
{
    ...
    static int contSplash_update_done_off = 0;
	...
	if (mdp_pdata) {
		if (mdp_pdata->cont_splash_enabled) {
			...
			if (!contSplash_update_done) {
                ...
                if( contSplash_update_done_off == 0 ) {
                    contSplash_update_done_off = 1;
                }
			}
		} else
	...
	mdp4_extn_disp = 0;
    if( contSplash_update_done_off == 1 ) {
        contSplash_update_done_off = 2;
        mdp_pipe_ctrl(MDP_CMD_BLOCK,
