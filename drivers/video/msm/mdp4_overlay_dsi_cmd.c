...
void mdp4_overlay0_done_dsi_cmd(struct mdp_dma_data *dma)
{
    ...
    DISP_LOCAL_LOG_EMERG("DISP mdp4_overlay0_done_dsi_cmd S\n");
	if (dsi_pipe->ov_blt_addr == 0) {
		mdp_pipe_ctrl(MDP_OVERLAY0_BLOCK, MDP_BLOCK_POWER_OFF, TRUE);
		spin_lock(&mdp_spin_lock);
		dma->busy = FALSE;
        ...
        DISP_LOCAL_LOG_EMERG("DISP mdp4_overlay0_done_dsi_cmd err1\n");
		return;
	}
	...
	if (diff >= 2) {
		mdp_disable_irq_nosync(MDP_OVERLAY0_TERM);
        DISP_LOCAL_LOG_EMERG("DISP mdp4_overlay0_done_dsi_cmd err2\n");
		return;
	}
	...
	mipi_dsi_cmd_mdp_start();
	mdp_disable_irq_nosync(MDP_OVERLAY0_TERM);
    DISP_LOCAL_LOG_EMERG("DISP mdp4_overlay0_done_dsi_cmd E\n");
}
...
void mdp4_dsi_blt_dmap_busy_wait(struct msm_fb_data_type *mfd)
{
    ...
    DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_blt_dmap_busy_wait Wait S\n");
	...
	if (need_wait) {
        ...
        DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_blt_dmap_busy_wait Wait COMP\n");
	}
    DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_blt_dmap_busy_wait Wait E\n");
}
...
void mdp4_dsi_cmd_dma_busy_wait(struct msm_fb_data_type *mfd)
{
    ...
    DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_cmd_dma_busy_wait Wait S\n");
	...
	if (need_wait) {
        ...
        DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_cmd_dma_busy_wait Wait COMP\n");
	}
	pr_debug("%s: done pid=%d dsi_clk_on=%d\n",
			 __func__, current->pid, mipi_dsi_clk_on);
    DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_cmd_dma_busy_wait Wait E\n");
}
				...
				struct mdp4_overlay_pipe *pipe)
{
    ...
    DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_cmd_overlay_kickoff S\n");
	/* start OVERLAY pipe */
	spin_unlock_irqrestore(&mdp_spin_lock, flag);
	mdp_pipe_kickoff(MDP_OVERLAY0_TERM, mfd);
	mdp4_stat.kickoff_ov0++;
    DISP_LOCAL_LOG_EMERG("DISP mdp4_dsi_cmd_overlay_kickoff E\n");
}
