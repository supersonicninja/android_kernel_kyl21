...
static int32_t kc_gyro_set_reset(int32_t reset)
{
	...
	if (reset) {
		if (yas_gyro_get_enable(data->driver)) {
			...
			ret = schedule_delayed_work(&data->delayed_work, delay_to_jiffies(delay) + 1);
			YLOGD(("%s(): schedule_delayed_work ret=%d\n",__func__,ret));
		}
	} else {
...
static void yas_gyro_delayed_work_func(struct work_struct *delayed_work)
{
	...
	if (delay > 0)
		schedule_delayed_work(&data->delayed_work, (unsigned long)delay_to_jiffies(delay));
	else
		YLOGD(("%s(): delay=%d\n",__func__,delay));
}
