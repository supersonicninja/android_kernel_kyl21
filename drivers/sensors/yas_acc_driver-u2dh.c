...
static int yas_u2dh_read_reg(unsigned char, unsigned char *, unsigned char);
static int yas_u2dh_write_reg(unsigned char, unsigned char *, unsigned char);
static int yas_u2dh_read_reg_byte(unsigned char);
static int yas_u2dh_write_reg_byte(int, int);
...
static int yas_u2dh_read_reg(unsigned char adr, unsigned char *buf,
			     unsigned char len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
...
static int yas_u2dh_write_reg(unsigned char adr, unsigned char *buf,
			      unsigned char len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
	int err = YAS_ERROR_DEVICE_COMMUNICATION;
...
static int yas_u2dh_read_reg_byte(unsigned char adr)
{
	unsigned char buf;
	int err;
...
static int yas_u2dh_write_reg_byte(int adr, int val)
{
	return yas_u2dh_write_reg((unsigned char)adr,
				  (unsigned char *)&val, 1);
}
